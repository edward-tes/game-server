...
var _ = require('underscore'),
    ...
    GameRatingDao = require('../common/dao/GameRatingDao'),
...
module.exports = function (app, db) {
    ...
    var gameRatingDao = new GameRatingDao(db);
        ...
        gameUserDao.getPwdByEmail(data.email, function (err, password) {
            if (err) {
                ...
                if (password && password == data.password) {
                    ...
                    date.setTime(date.getTime() + expireDays * 24 * 3600 * 1000);
                    res.setHeader('Set-Cookie', ['email=' + data.email + ';httpOnly=true;Expires=' + date.toGMTString() + ',password=' + password , ';httpOnly=true;Expires=' + date.toGMTString() + ';httpOnly=true']);
                    res.send({code: 200});
                } else {
                    res.send({code: 400});
                }
    ...
    app.post('/game/rating/add', function (req, res, next) {
        var data = req.body;
        var self = this;
        data.rating = parseFloat(data.rating);
        gameRatingDao.getRatingByUserIdAndGameId(data.gameId, data.userId, function (err, doc) {
            if (err) {
                var error = new Error();
                error.code = ERROR_CODE.GAME_GET_RATING_ERROR;
                next(err);
            } else {
                if (!doc) {
                    gameRatingDao.addRating(data, function (err, doc) {
                        if (err) {
                            var error = new Error();
                            error.code = ERROR_CODE.GAME_ADD_RATING_ERROR;
                            next(err);
                        } else {
                            console.log(doc.gameId);
                            gameRatingDao.getRatingByGameId(doc.gameId, function (err, docs) {
                                if (err) {
                                    error = new Error();
                                    error.code = ERROR_CODE.GAME_GET_RATING_ERROR;
                                    next(err);
                                    ...
                                    for (var i = 0; i < docs.length; i++) {
                                        allRating += docs[i].rating;
                                    }
                                    console.log(allRating);
                                    gameDao.updateRating(ObjectID.createFromHexString(doc.gameId), (allRating / docs.length).toFixed(1), function (err, doc) {
                                        if (err) {
                                            error = new Error();
                                            error.code = ERROR_CODE.GAME_UPDATE_RATING_ERROR;
                                            next(err);
                                        } else {
                                            res.send({success: 1});
                                        }
                ...
                } else {//if current user is rated this game, can't repeat rating
                    res.send({success: 0,message:'illegal operation'});
                }
            }
        });

    });
    app.post('/game/rating/isok', function (req, res, next) {
        var data = req.body;
