...
GameDao.prototype.getHotGameList = function(){

};

GameDao.prototype.updateRating = function(id,rating, cb){
    this.tGameData_.update({_id:id}, {$set:{rating:rating}},function (err, doc) {
        if (!err)
            cb(null, doc);
        else {
            cb(err);
