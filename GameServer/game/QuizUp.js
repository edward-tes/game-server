...
QuizUp.prototype.onFinishSelect_ = function (e) {
    ...
    console.log("is enable ai " + this.finishedNumber_ );
    ...
    if (this.finishedNumber_ == this.maxPlayerNumber_) {
        BOK.each(this.playerCons_, function (con) {
            ...
            this.questionDao_.getTopicByTopic(con.getTopic(), BOK.createDelegate(this, function (err, doc) {
                var history = {topic: doc.text, playerId: myName, oppId: oppName, score: this.result_[myName].score, oppScore: this.result_[oppName].score, correct: this.result_[myName].correctNum || 0, total: this.questions_.length, time: new Date().getTime(), questions: this.result_[myName].questions};
...
QuizUp.prototype.onRematchSelectTopic_ = function (e) {
    ...
    } else { // the last game player play with ai , so it also rematch with ai
        BOK.each(this.playerCons_, function (con) {
           // con.rematchTopicResult(player.id, e.body.topic, true);
            this.type = e.body.topic;
            con.setTopic(e.body.topic);
            this.result_ = {};
            this.finishedNumber_ = 0;
            this.gameStart_([player]);
            //this.removeAllConHandlers_(con);
        }, this);
    }
};
 ...
 * @param {number} quizTime //the player thinking time
 * @private
 */
